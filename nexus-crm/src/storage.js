...
export async function guardarAgencias(agencias) {
  ...
  if (ids.length) {
    // Solo borramos agencias "comunes". Los sindicatos los crea y administra el
    // módulo de Sindicatos (escriben su fila directo), así que NUNCA los tocamos
    // desde el autosave del padrón general.
    await supabase.from("agencias").delete().eq("tipo", "agencia").not("id", "in", `(${ids.join(",")})`);
  }
}
